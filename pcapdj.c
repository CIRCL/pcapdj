...
int main(int argc, char* argv[])
{
    ...
    cpubstr = calloc(128,1);
    assert(cpubstr);

    redis_srv_port = 6379;        
    while ((opt = getopt(argc, argv, "b:hs:p:z:d:c:")) != -1) {
        switch (opt) {
            case 's':
    ...
    fprintf(stderr, "[INFO] Consumer ZMQ stream = %s\n", cpubstr);
